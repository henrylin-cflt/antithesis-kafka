...
impl TestProducer {
    pub fn new(
        ...
        producer_config
            ...
            .set("transaction.timeout.ms", "2000")
        ...
        if config.enable_transactions {
            inner_producer
                .init_transactions(Timeout::After(Duration::from_millis(1000)))
    ...
    async fn produce_sequence_with_transaction(
        ...
        for next_value in sequence {
            ...
            loop {
                ...
                match self
                    .inner_producer
                    .send(record, Timeout::After(Duration::from_millis(1000)))
                    .await
                {
                    Ok((partition, offset)) => {
                        messages_sent += 1;
    ...
    async fn produce_sequence_without_transaction(
        ...
        for next_value in sequence {
            ...
            loop {
                ...
                match self
                    .inner_producer
                    .send(record, Timeout::After(Duration::from_millis(1500)))
                    .await
                {
                    Ok((partition, offset)) => {
                        let mut global_state = self.global_state.write().unwrap();
